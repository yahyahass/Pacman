...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for position in self.allPositions:
            ...
            self.beliefs[position] = self.beliefs[position] * probability
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState):
        ...
        for i in self.allPositions:
            ...
            for j in dist:
                result[j] += dist[j] * self.beliefs[i]
        result.normalize()
        self.beliefs = result
        self.beliefs.normalize()
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        currPacman = gameState.getPacmanPosition()
        currJail = self.getJailPosition()
        dist = self.getBeliefDistribution()
        for x in dist:
            dist[x] *= self.getObservationProb(observation,currPacman,x, currJail)
        if dist.total() == 0:
            self.initializeUniformly(gameState)
        else:
            distList = list(dist)
            self.particles = random.choices(distList, k=self.numParticles,weights=list(dist.values()))
    ...
    def elapseTime(self, gameState):
        ...
        for i in self.particles:
            pos_distr = self.getPositionDistribution(gameState, i)
            temp = pos_distr.sample()
