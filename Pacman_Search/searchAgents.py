...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.EAST, Directions.WEST, Directions.NORTH, Directions.SOUTH]:
            ...
            if not hitsWall:
                temp = (nextx, nexty)
                for xcor, ycor in enumerate(self.corners):
                    if temp == ycor and not corner[xcor]:
                        cornerhit = list(corner[:])
                        cornerhit[xcor] = True
                        cornerhit = tuple(cornerhit)
                successors.append(((temp, cornerhit), action, 1))
        self._expanded += 1
        return successors
...
def cornersHeuristic(state, problem):
    ...
    curr,visitedSet=state
    distance = -1
    corner = -1

    for i, j in enumerate(corners):
        if not  visitedSet[i]:
            temp = (curr[0] - j[0]) + (curr[1] - j[1])
            if temp > distance:
                distance = temp
                corner = i
...
def foodHeuristic(state, problem):
    ...
    for i in foodMap:
        for j in foodMap:
            temp = util.manhattanDistance(i, j) 
            if (temp > distance):
                distance = temp
                xcor = i
                ycor = j

    distancex = util.manhattanDistance(position, xcor)
    distancey = util.manhattanDistance(position, ycor)

    return min(distancex, distancey) + distance
